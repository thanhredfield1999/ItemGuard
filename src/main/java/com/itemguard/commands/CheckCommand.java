...
public class CheckCommand implements CommandExecutor, TabCompleter {
    ...
    public void checkItemInHand(Player player) {
        ...
        plugin.getDB().getItemAsync(code).thenCombine(
            plugin.getDB().getHistoryCountAsync(code),
            java.util.AbstractMap.SimpleImmutableEntry::new
        ).whenComplete((loaded, failure) -> UiMainThreadHandoff.dispatch(plugin, () -> {
            if (failure != null) {
                plugin.getLogger().log(java.util.logging.Level.SEVERE,
                    "Failed to load ItemGuard check data", failure);
                player.sendMessage("§e§l[ItemGuard] §cKhông thể kiểm tra lúc này. Vui lòng thử lại.");
                return;
            }
            Optional<ItemData> dataOpt = loaded.getKey();
            if (dataOpt.isEmpty()) {
                plugin.getMessages().send(player, "item-not-tracked");
                return;
            }
            renderCheck(player, code, hand, dataOpt.orElseThrow(), loaded.getValue());
        }));
    }

    private void renderCheck(Player player, String code, ItemStack hand, ItemData data, int historyCount) {
