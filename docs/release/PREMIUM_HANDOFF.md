...
| `anti-dupe.observation-retention-minutes` | `30` | How long observation rows are kept. They answer "two places in this audit" and "which servers has this identity been seen on inside a window", so keep this at least as large as `multi-server.cross-server-window-minutes`; rows older than the window are deleted, rows inside it never are, whoever wrote them. Floored at two scan cycles. |
